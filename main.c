...
void isWrongWord()
{
  for (int i = 0; i < num_t; i++)
  {
    ...
    {
      printf("%s\n", text[i]);
      strcpy_s(output[num_o], strlen(text[i])+1, text[i]); // strcpy_s就这么用
      num_o++;
    }
  }
  return;
...
int readDict(char * fn)
{
  ...
  int dictkey = 0;
  // 打开文件
  if (fopen_s(&fp, fn, "r") != 0)
  {
    printf("open readDict file error!\n");
    return -1;
  }
  // 读取一行到buf，每次调用文件指针fp会自动后移一行
  while (fgets(dict[count], MAX_LEN, fp) != NULL) // 文件读取结束则跳出循环
  {
...
int readtext(char *fn)
{
  ...
  while (fgets(row, MAX_LEN, fp) != NULL)
  {
    ...
    for (unsigned int i = 0; i < strlen(row); i++)
    {
      ...
      else if ((row[i] >= 'a' && row[i] <= 'z') || (row[i] >= 'A' && row[i] <= 'Z') || row[i] == '\'')
      {
...
int main()
{
  char dictionaryFilename[100] = "res/dictionary.txt";
  num_d = readDict(dictionaryFilename);
  char textFilename[100] = "res/lalaland.txt";
